...
const EditProfile = () => {
    ...
    const loadUser = async () => {
      try {
        ...
        if (responseData) {
          ...
          setImageSrc(
            `https://res.cloudinary.com/${process.env.REACT_APP_CLOUDINARY_CLOUD_NAME}/image/upload/${responseData.user.imageUrl}`
          );
  ...
  const handleUploadImage = async () => {
    let public_id = loadedUser.imageUrl;
    if (imageSrc && isImageChanged) {
    ...
    try {
      // Check if imageUrl is not empty before proceeding
      const songsList = [
        {
          name: song1Name,
          url: song1Url,
      ...
      const formData = {
        ...
        profession,
        songs: songsList,
        imageUrl: public_id, // Use the received public_id
        genres,
      };

      // Now call handleAuth with the updated formData
      handleAuth(
